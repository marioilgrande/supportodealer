// Elenco negozi -> [SIS-SUB, Agenzia]. Fonte: Pannello Segnalazioni (segnalazioni.xlsx).
// In futuro sostituibile con lettura dal DB Neon condiviso.
export const NEGOZI = {
...
// Match "morbido": accetta nomi abbreviati o parziali (es. "dama" -> "DAMA SNC DI ...").
export function matchStore(input) {
  const q = normalizeName(input);
  if (!q) return null;
  const keys = Object.keys(NEGOZI);
  for (const k of keys) { if (normalizeName(k) === q) return k; }
...
export function codiciNegozio(input) {
  const key = matchStore(input);
  if (!key) return { trovato: false, nome: input || '', sisSub: '', agenzia: '' };
  return { trovato: true, nome: key, sisSub: NEGOZI[key][0] || '', agenzia: NEGOZI[key][1] || '' };
}
