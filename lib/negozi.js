...
function matchStoreIn(input, keys) {
...
export async function matchStore(input) {
  const negozi = await getNegozi();
  return matchStoreIn(input, Object.keys(negozi));
}

export async function codiciNegozio(input) {
  const negozi = await getNegozi();
  const key = matchStoreIn(input, Object.keys(negozi));
