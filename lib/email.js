// Invio email tramite Brevo (https://brevo.com) — free 300 email/giorno.
// Il mittente si verifica con un clic (Single Sender), senza toccare il DNS.
// Env: BREVO_API_KEY, MAIL_FROM_EMAIL (mittente verificato su Brevo),
//      MAIL_FROM_NAME (facolt.), MAIL_ADMIN, MAIL_SIMONE.

export async function sendEmail({ to, subject, html }) {
  const apiKey = process.env.BREVO_API_KEY;
  const senderEmail = process.env.MAIL_FROM_EMAIL || process.env.MAIL_ADMIN;
  const senderName = process.env.MAIL_FROM_NAME || 'Supporto Dealer';
  if (!apiKey || !senderEmail) {
    console.warn('BREVO_API_KEY o MAIL_FROM_EMAIL mancante: email non inviata');
